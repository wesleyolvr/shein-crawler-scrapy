...
class SheinProductsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        if self.categorys[0]:
            for category in self.categorys:
                ...
                if not self.redis_cache.check_cache(url_category):
                    yield scrapy.Request(
                        url=url_category, callback=self.parse, headers=self.headers
                    )
                else:
                    self.logger.info(
            ...
            self.logger.info(f"A URL não foi passada.")
    ...
    def parse_all_products(self, response):
        if self.redis_cache.check_cache(response.url):
            self.logger.info(f"Dados já presentes no cache para a URL: {response.url}")
            return
        ...
        if products:
            for product in products:
                item = ProductItem(
                    ...
                    price_real_symbol=product["retailPrice"]["amountWithSymbol"],
                    price_real=product["retailPrice"]["amount"],
                    price_us_symbol=product["retailPrice"]["usdAmountWithSymbol"],
                    price_us=product["retailPrice"]["usdAmount"],
                    discountPrice_real_symbol=product["salePrice"]["amountWithSymbol"],
                    ...
                    datetime_collected=datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                )
                yield item
            self.redis_cache.set_cache(self.categorys[0], "1")
        else:
            self.logger.info(f"Nenhum produto encontrado para a URL: {response.url}")
